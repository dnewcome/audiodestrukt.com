...
I was looking around for a sampler instrument on Linux to test my Pedal Fighter project with, and while I remembered Specimen from a while ago, I remembered that there weren’t many samplers out there apart from the [Linux Sampler project](http://www.linuxsampler.org/) (which I’ve had problems with in the past – way too complicated for quick setups).

I dip my toes into the Linux audio development world every now and then, so I figured I’d take a look at the current state-of-the-art for Linux instrument plugins, [DSSI](http://dssi.sourceforge.net/) (dizzy).

DSSI is basically [VSTi](http://en.wikipedia.org/wiki/Virtual_Studio_Technology) for Linux, or [LADSPA](http://www.ladspa.org/) for instruments depending on your viewpoint. It is a [spec](http://dssi.sourceforge.net/RFC.html) that we can conform with to enable our code to be hosted in different programs such as [Ardour](http://ardour.org/).

I grabbed the DSSI development kit and compiled it. I had to grab the LADSPA SDK using [APT](http://en.wikipedia.org/wiki/Advanced_Packaging_Tool) (I’m using Ubuntu)
...
```
There are some other dependencies that I already had installed, but are worth mentioning. Liblo is the [Lightweight OSC library](http://liblo.sourceforge.net/).
...
```
_init(), is part of [dlopen](http://linux.die.net/man/3/dlopen), which is used to load dynamic libraries at runtime. The init function constructs the plugin descriptors according to the LADSPA and DSSI specs. 

The following function is specified in the [LADSPA spec](http://gdam.ffem.org/ladspa-doc/ladspa-2.html), and returns the descriptor when called.
