...
I have been playing around with generating MIDI files using Node.js recently. I’m using a minimal MIDI file writer called [node-midimal](https://github.com/sabberworm/node-midimal). Everything was working fine until I tried to output a drum beat that had a hihat and a snare/kick playing on the same beat.
...
![Screen Shot 2014-06-10 at 8.09.15 PM](images/2014-06-10_subtleties-of-standard-midi-file-delta-time_screen-shot-2014-06-10-at-8-09-15-pm.png)

I changed the second note to play correctly in Logic and saved the file out as another MIDI file. 

![Screen Shot 2014-06-10 at 8.59.38 PM](images/2014-06-10_subtleties-of-standard-midi-file-delta-time_screen-shot-2014-06-10-at-8-59-38-pm.png)

Then I grabbed a little MIDI file dumper tool called [Daktari MIDI](http://interglacial.com/pub/dr_midi.html), which is sort of like a disassembler for MIDI files, and dumped out both files.
...
The other is that the note events are in slightly different orders. That’s when it hit me. Delta time applies for all events, and a note off event is still treated as an event that advances the time base. In order for two notes to play at the same time, both note off events have to happen sequentially before any note-off events. Adding a note using [track.note()](https://github.com/sabberworm/node-midimal/blob/master/lib/writer.js#L106) adds both note-on and note-off, which means that the next note added will start at least where the other ended.
