...
I’ve [written a little bit](https://audiodestrukt.wordpress.com/2012/12/13/the-state-of-usb-audio-on-windows/) before about the state of USB audio in Windows. It kind of sucks actually, and it seems to suck a little bit more on Windows 7 than it does in Windows 8, which is kind of convenient, as I’d rather go with the later version of Windows if possible. Note that the [dpclat](http://www.thesycon.de/deu/latency_check.shtml) tool gives false readings on Windows 8. This caused me pause for a while since the average reported DPC latency will be very high with respect to Windows 7. However I wasn’t seeing as many problems as I would have expected for this high latency. I found a reference to dpclat being updated for Windows 8 support and that the readings are artificially high due to a change in the way Windows reports the DPC statistics. Check out [this Anandtech thread](http://forums.anandtech.com/archive/index.php/t-2250925.html) for details.
...
The Mac works with USB audio without a hitch out of the box in my limited testing. On Windows the onus is on us to figure out which drivers or devices are killing our performance. This is a tedious process of using [LatencyMon](http://www.resplendence.com/latencymon) to figure out which drivers are taking most of the CPU’s interrupt handling time and deferred procedure calls. These are important because once a driver is in a DPC or is handling an interrupt it cannot be preempted. So if a driver is spending a lot of time handling interrupts, there is a good chance that at some point the audio driver will need CPU time to flush its buffers and not be able to get it because the CPU is servicing non-preemptible requests.
...
In one case “ACPI Compliant Control Method Battery” was a huge culprit. The driver ACPI.sys was scheduling a lot of DPCs according to LatencyMon and I couldn’t figure out what to do about it until I found [this post](http://www.native-instruments.com/forum/showthread.php?t=130166) on the NI forums. Once I disabled the ACPI battery device things got a lot better.
...
I had some issues with the Windows scheduled tasks like the Windows Assessment Tool running suddenly and causing CPU spikes. Check out [this link](http://social.technet.microsoft.com/Forums/en/w7itproui/thread/8ec7dbae-4cfc-413a-b619-c63b3b610d5a) for details on how do disable Winsat in the scheduled tasks console. Surely Windows search service could cause problems, and a probably a host of others.
