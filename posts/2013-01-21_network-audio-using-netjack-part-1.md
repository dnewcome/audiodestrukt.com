...
I’ve been playing with networked audio again after some [previous](https://audiodestrukt.wordpress.com/2011/11/01/jamlan-update/) unsatisfactory [attempts](https://audiodestrukt.wordpress.com/2011/10/30/local-network-audio/) at getting something working using [pulseaudio](http://www.freedesktop.org/wiki/Software/PulseAudio) and [RTP streaming](http://en.wikipedia.org/wiki/Real-time_Transport_Protocol). I tried a few things in the past like using PD’s [netsend~ object](http://www.nullmedium.de/dev/netsend~/) but nothing gave me decent results.

I tried a few things this time around like [Wormhole](http://code.google.com/p/wormhole2/) which is a VST plugin that lets you do client-server networked audio between the points that you have inserted the VST in your host.
...
I’ve been kind of avoiding [Jack](http://jackaudio.org/) on platforms other than Linux up until this point, but I feel like I’ve exhausted all my other options. I’ve gotten Jack installed on my Mac and on my Windows 8 machine, and my goal is to be able to route audio between them.
...
![procexp-reaper](images/2013-01-21_network-audio-using-netjack-part-1_procexp-reaper.png)
...
![reaper-connections2](images/2013-01-21_network-audio-using-netjack-part-1_reaper-connections2.png)
...
The next step then is to run the Jack server using some particular settings to try to force ASIO. There are some hints about this [here on the Jack site](http://jackaudio.org/jack_on_windows). Running the server like the following will start up ASIO4ALL when starting Jack:
...
![jack-setup](images/2013-01-21_network-audio-using-netjack-part-1_jack-setup.png)

The following screenshot shows JackRouter listed as an audio device in Mac audio and MIDI preferences:

![audio-midi-setup](images/2013-01-21_network-audio-using-netjack-part-1_audio-midi-setup.png)
