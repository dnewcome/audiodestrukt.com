...
I’m starting to set up a rig for doing live performance streams. In order to do this I’m feeding the output of my setup to OBS. There are a few tricks for getting this to work the way I wanted it to. The main thing is that I have a multi-channel audio interface (Focusrite Scarlett) and I want to use inputs other than the first two. For some reason OBS will only support the first two inputs of an interface out of the box. Fortunately there is an ASIO plugin available https://github.com/Andersama/obs-asio/releases
