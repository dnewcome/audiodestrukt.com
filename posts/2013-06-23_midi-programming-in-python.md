...
I’ve been playing with different ways to [script MIDI I/O](https://audiodestrukt.wordpress.com/2012/11/26/midi-with-ruby/) over the last year or so. There are a ton of different ways to skin this cat including special-purpose languages like [PD](http://puredata.info/) and [KeyKit](http://nosuch.com/keykit/). However, it’s nice to be able to do some coding in a more general purpose language if for no other reason that other programmers can see what the hell you’re trying to do without wrapping their head around some bizarre environment.

What I’ve found is that for the most part, MIDI has been relegated to game libraries and low-level libraries like [PortMidi](http://portmedia.sourceforge.net/portmidi/) and [RtMidi](http://www.music.mcgill.ca/~gary/rtmidi/). Some of these libraries are also kind of old and rely on outdated midi device interfaces ([OSS](http://www.opensound.com/), etc.).
...
Note that I installed directly from bitbucket, due to an [error in the build](https://bitbucket.org/pygame/pygame/issue/139/sdlh-not-found-even-thought-it-exists) that has been fixed in the latest sources.

The [pygame MIDI docs](http://www.pygame.org/docs/ref/midi.html), along with some sample pygame code got me as far as reading my Korg NanoPad and dumping the MIDI events to the console. Here is the code:
...
```
I came across some [interesting code](https://bitbucket.org/pygame/pygame/src/25e3f2cee879/examples/midi.py) in the course of getting this stuff working. There is apparently a pygame event type MIDI that can be consumed just like a mouse move or keypress event. However it looks like we still need to pull the data using read() anyway, so I’m not sure if this is useful.
